...
class RegNetUS0_Coarse2Fine(nn.Module):
    def __init__(self, origin_size=False, dp_ratio=0.0, image_scale=0.25):
        ...
        self.conv7 = nn.Sequential(
            nn.ConvTranspose3d(
                128, 32, kernel_size=3, padding=1, output_padding=1, stride=2, bias=False
            ),
            nn.BatchNorm3d(32),
        ...
        self.conv9 = nn.Sequential(
            nn.ConvTranspose3d(
                97, 16, kernel_size=3, padding=1, output_padding=1, stride=2, bias=False
            ),
            nn.BatchNorm3d(16),
            nn.ReLU(inplace=True),
        )
        ...
        self.conv11 = nn.Sequential(
            nn.ConvTranspose3d(
                49, 8, kernel_size=3, padding=1, output_padding=1, stride=2, bias=False
            ),
            nn.BatchNorm3d(8),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, x_list):
        ...
        x = torch.cat(
            [x, x3, F.interpolate(prob4, scale_factor=2, mode="trilinear", align_corners=True)], 1
        )
        prob3 = self.dropout3(self.prob3(x))
        # prob3 = self.prob3(x)
        x = self.conv9(x) + self.conv2(conv1)
        x = torch.cat(
            [x, x2, F.interpolate(prob3, scale_factor=2, mode="trilinear", align_corners=True)], 1
        )
        prob2 = self.dropout2(self.prob2(x))
        # prob2 = self.prob2(x)
        x = self.conv11(x) + conv0
        x = torch.cat(
            [x, x1, F.interpolate(prob2, scale_factor=2, mode="trilinear", align_corners=True)], 1
        )
