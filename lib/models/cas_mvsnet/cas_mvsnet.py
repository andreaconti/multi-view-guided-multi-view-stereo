...
class DepthNet(nn.Module):
    ...
    def forward(
        ...
        if hints is not None and validhints is not None:
            ...
            depth_hyps = (
                depth_values.unsqueeze(1).expand(batch_size, feats, -1, height, width).detach()
            )
...
class CascadeMVSNet(nn.Module):
    def __init__(
        ...
        self.feature = FeatureNet(
            base_channels=8, stride=4, num_stage=self.num_stage, arch_mode=self.arch_mode
        )
        if self.share_cr:
    ...
    def forward(self, imgs, proj_matrices, depth_values, hints=None, validhints=None):
        ...
        for stage_idx in [2, 1, 0]:  # range(self.num_stage):
            ...
            stage_scale = self.stage_infos["stage_{}".format(stage_idx)]["scale"]
            ...
            depth_range_samples = get_depth_range_samples(
                cur_depth=cur_depth,
                ndepth=self.ndepths[2 - stage_idx],
                depth_inteval_pixel=self.depth_interals_ratio[2 - stage_idx] * depth_interval,
                dtype=img[0].dtype,
                device=img[0].device,
            ...
            outputs_stage = self.DepthNet(
                ...
                num_depth=self.ndepths[2 - stage_idx],
            ...
            depth = outputs_stage["depth"]  # .unsqueeze(1)

            outputs["stage_{}".format(stage_idx)] = depth.unsqueeze(1)
        ...
        return {
            "depth": outputs["refined_depth"] if self.refine else outputs,
            "loss_data": {
                "depth": outputs,
            },
