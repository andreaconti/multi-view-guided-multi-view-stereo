...
def homo_warping_depthwise(src_fea, src_proj, ref_proj, depth_value):
    ...
    warped_src_fea = F.grid_sample(
        src_fea, grid.view(batch, 1 * height, width, 2), mode="bilinear", padding_mode="zeros"
    ).type(torch.float32)
...
def homo_warping_depthwise_ori(src_fea, src_proj, ref_proj, depth_value):
    ...
    with torch.no_grad():
        ...
        xyz = torch.unsqueeze(xyz, 0).repeat(batch, 1, 1)  # [B, 3, H*W]
        rot_xyz = torch.matmul(rot, xyz)  # [B, 3, H*W]
        rot_depth_xyz = rot_xyz.unsqueeze(2).repeat(1, 1, 1, 1) * depth_value.view(
            batch, 1, 1, 1
        )  # [B, 3, Ndepth, H*W]
...
def homo_warping2(src_fea, src_proj, ref_proj, depth_values):
    ...
    with torch.no_grad():
        ...
        for i in range(num_depth):
            rot_depth_xyz = rot_xyz.unsqueeze(2).repeat(1, 1, 1, 1) * depth_values[:, i].view(
                batch, 1, 1, 1
            )  # [B, 3, 1, H*W]
            ...
            warped_src_fea[:, :, i, :, :] = F.grid_sample(
                src_fea, grid.view(batch, height, width, 2), mode="bilinear", padding_mode="zeros"
            )
...
def homo_warping3(src_fea, src_proj, ref_proj, depth_values):
    ...
    with torch.no_grad():
        ...
        rot_depth_xyz = rot_xyz.unsqueeze(2).repeat(1, 1, 1, 1) * (
            depth_values.view(batch, 1, 1, 1)
        )  # [B, 3, 1, H*W]
        # print('rot_depth_xyz: ', np.shape(rot_depth_xyz))
        proj_xyz = rot_depth_xyz + trans.view(batch, 3, 1, 1)  # [B, 3, 1, H*W]
        # print('proj_xyz: ', np.shape(proj_xyz))
...
if __name__ == "__main__":
    ...
    dataset = MVSDataset(
        "/data1/Dataset/mvs_training/dtu/", "../lists/dtu/train.txt", "train", 3, 256
    )
    ...
    cv2.imwrite(
        "../tmp/ref.png", ref_img.permute([0, 2, 3, 1])[0].detach().cpu().numpy()[:, :, ::-1] * 255
    )
    cv2.imwrite(
        "../tmp/src.png",
