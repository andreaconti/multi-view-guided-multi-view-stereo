...
class FeatNet(nn.Module):
    def __init__(self, gn):
        ...
        if gn:
            ...
            self.conv0_1 = convgnrelu(
                base_filter * 2, base_filter * 4, kernel_size=3, stride=1, dilation=1
            )
            self.conv0_2 = convgnrelu(
                base_filter * 4, base_filter * 4, kernel_size=3, stride=1, dilation=2
            )
            self.conv0_3 = convgnrelu(
                base_filter * 4, base_filter * 4, kernel_size=3, stride=1, dilation=1
            )

            # conv1_2 with conv0_2
            self.conv1_1 = convgnrelu(
                base_filter * 4, base_filter * 4, kernel_size=3, stride=1, dilation=3
            )
            self.conv1_2 = convgnrelu(
                base_filter * 4, base_filter * 4, kernel_size=3, stride=1, dilation=1
            )

            # conv2_2 with conv0_2
            self.conv2_1 = convgnrelu(
                base_filter * 4, base_filter * 4, kernel_size=3, stride=1, dilation=4
            )
            self.conv2_2 = convgnrelu(
                base_filter * 4, base_filter * 4, kernel_size=3, stride=1, dilation=1
            )
...
class FeatNet_new(nn.Module):
    def __init__(self, gn):
        ...
        if gn:
            ...
            self.conv0_1 = convgnrelu(
                base_filter, base_filter * 2, kernel_size=3, stride=1, dilation=1
            )
            self.conv0_2 = convgnrelu(
                base_filter * 2, base_filter * 4, kernel_size=3, stride=1, dilation=1
            )

            self.conv1_0 = convgnrelu(
                base_filter * 4, base_filter * 8, kernel_size=3, stride=1, dilation=2
            )
            self.conv1_1 = convgnrelu(
                base_filter * 8, base_filter * 8, kernel_size=3, stride=1, dilation=1
            )
            self.conv1_2 = convgnrelu(
                base_filter * 8, base_filter * 8, kernel_size=3, stride=1, dilation=1
            )
...
class FeatNet2(nn.Module):
    def __init__(self, gn):
        ...
        if gn:
            ...
            self.conv_1 = convgnrelu(
                base_filter, base_filter * 2, kernel_size=3, stride=1, dilation=1
            )
            self.conv_2 = convgnrelu(
                base_filter * 2, base_filter * 4, kernel_size=3, stride=1, dilation=2
            )

            self.conv0_0 = convgnrelu(
                base_filter * 4, base_filter * 4, kernel_size=5, stride=2, dilation=1
            )
            self.conv0_1 = convgnrelu(
                base_filter * 4, base_filter * 4, kernel_size=3, stride=1, dilation=1
            )
...
class UNetConvLSTM(nn.Module):  # input 3D feature volume
    def __init__(
        ...
        if self.gn:
            for i in range(0, self.num_layers):
                ...
                cell_list.append(
                    ConvLSTMCell(
                        input_size=(int(self.height / scale), int(self.width / scale)),
                        scale=scale,
            ...
            self.deconv_0 = deConvGnReLU(
                16, 16, kernel_size=3, stride=2, padding=1, bias=self.bias, output_padding=1  # 16
            )
            ...
            self.deconv_0 = deConvBnReLU(
                16, 16, kernel_size=3, stride=2, padding=1, bias=False, output_padding=1  # 16
            )
    ...
    def forward(self, input_tensor, hidden_state=None, idx=0, process_sq=True):
        ...
        if idx == 0:  # input the first layer of input image
            ...
            hidden_state = self._init_hidden(batch, height, width)
        ...
        if process_sq:
            ...
            h0, c0 = hidden_state[0] = self.cell_list[0](
                input_tensor=cur_layer_input, cur_state=hidden_state[0]
            )
            # self.hidden_state[0] = (h0, c0)

            h0_1 = nn.MaxPool2d((2, 2), stride=2)(h0)
            h1, c1 = hidden_state[1] = self.cell_list[1](
                input_tensor=h0_1, cur_state=hidden_state[1]
            )
            # self.hidden_state[1] = (h1, c1)
            h1_0 = nn.MaxPool2d((2, 2), stride=2)(h1)
            h2, c2 = hidden_state[2] = self.cell_list[2](
                input_tensor=h1_0, cur_state=hidden_state[2]
            )
            # self.hidden_state[2] = (h2, c2)
            h2_0 = self.deconv_0(h2)  # auto reuse

            h2_1 = torch.cat([h2_0, h1], 1)
            h3, c3 = hidden_state[3] = self.cell_list[3](
                input_tensor=h2_1, cur_state=hidden_state[3]
            )
            # self.hidden_state[3] = (h3, c3)
            h3_0 = self.deconv_1(h3)  # auto reuse
            h3_1 = torch.cat([h3_0, h0], 1)
            h4, c4 = hidden_state[4] = self.cell_list[4](
                input_tensor=h3_1, cur_state=hidden_state[4]
            )
            ...
            for t in range(seq_len):
                # if t == 0:
                h0, c0 = self.cell_list[0](
                    input_tensor=cur_layer_input[:, t, :, :, :], cur_state=hidden_state[0]
                )
    ...
    def _init_hidden(self, batch_size, height=None, width=None):
        init_states = []
...
class UNetConvLSTMV4(UNetConvLSTM):
    ...
    def forward(self, input_tensor, hidden_state=None, idx=0, process_sq=True):
        ...
        if process_sq:
            ...
            h3, c3 = hidden_state[3] = self.cell_list[3](
                input_tensor=h2_0, cur_state=hidden_state[3]
            )
            ...
            h5, c5 = hidden_state[5] = self.cell_list[5](
                input_tensor=h4_1, cur_state=hidden_state[5]
            )

            # self.hidden_state[3] = (h3, c3)
            h5_0 = self.deconv_2(h5)  # auto reuse
            h5_1 = torch.cat([h5_0, h0], 1)
            h6, c6 = hidden_state[6] = self.cell_list[6](
                input_tensor=h5_1, cur_state=hidden_state[6]
            )
            # self.hidden_state[4] = (h4, c4)
...
class UNetPPConvLSTMV3(UNetConvLSTM):
    def __init__(
        ...
        super(UNetPPConvLSTMV3, self).__init__(
            input_size,
            input_dim,
            hidden_dim,
            kernel_size,
    ...
    def forward(self, input_tensor, hidden_state=None, idx=0, process_sq=True):
        ...
        if process_sq:
            ...
            h11, c11 = hidden_state[4] = self.cell_list[4](
                input_tensor=h1_2, cur_state=hidden_state[4]
            )
            # self.hidden_state[4] = (h4, c4)
            ...
            h4, c4 = hidden_state[5] = self.cell_list[5](
                input_tensor=h3_1, cur_state=hidden_state[5]
            )
...
class UNetPPConvLSTMV3UPS(UNetConvLSTM):
    def __init__(
        ...
        super(UNetPPConvLSTMV3UPS, self).__init__(
            input_size,
            input_dim,
            hidden_dim,
            kernel_size,
...
class CostAggNet(nn.Module):
    def __init__(
        self, input_size, input_dim, hidden_dim, kernel_size, num_layers, bias=True, gn=True
    ):
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        for i in range(0, self.num_layers):
            hidden_state[i] = self.gatenet[i](input_tensor=input_tensor, cur_state=hidden_state[i])
