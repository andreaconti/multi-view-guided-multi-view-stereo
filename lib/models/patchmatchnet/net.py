...
class FeatureNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        intra_feat = F.interpolate(intra_feat, scale_factor=2, mode="bilinear") + self.inner2(
            conv4
        )
        del conv4
...
class Refinement(nn.Module):
    ...
    def forward(
        ...
        depth = depth * (
            depth_max.view(batch_size, 1, 1, 1) - depth_min.view(batch_size, 1, 1, 1)
        ) + depth_min.view(batch_size, 1, 1, 1)
...
class PatchmatchNet(nn.Module):
    ...
    def forward(
        ...
        assert len(imgs_0) == len(
            self.proj_matrices_0
        ...
        photometric_confidence = F.interpolate(
            photometric_confidence, scale_factor=2, mode="nearest"
        )
