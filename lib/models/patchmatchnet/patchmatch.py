...
class DepthInitialization(nn.Module):
    ...
    def forward(
        ...
        if random_initialization:
            ...
            depth_sample = inverse_max_depth.view(
                batch_size, 1, 1, 1
            ) + depth_sample / patchmatch_num_sample * (
                inverse_min_depth.view(batch_size, 1, 1, 1)
                - inverse_max_depth.view(batch_size, 1, 1, 1)
            )
            ...
            if self.patchmatch_num_sample == 1:
                ...
                for k in range(batch_size):
                    depth_clamped.append(
                        torch.clamp(
                            depth_sample[k], min=inverse_max_depth[k], max=inverse_min_depth[k]
                        ).unsqueeze(0)
                    )
...
class Evaluation(nn.Module):
    ...
    def __init__(
        self, G: int = 8, stage: int = 3, evaluate_neighbors: int = 9, iterations: int = 2
    ) -> None:
        """Initialize method
    ...
    def forward(
        ...
        hints: torch.Tensor = None,
        validhints: torch.Tensor = None,
        ...
        if self.stage == 3 and view_weights is None:
            ...
            for src_feature, src_proj in zip(src_features, src_projs):
                ...
                warped_feature = warped_feature.view(
                    batch, self.G, feature_channel // self.G, num_depth, height, width
                )
                ...
                if self.training:
                    similarity_sum = similarity_sum + similarity * view_weight.unsqueeze(
                        1
                    )  # [B, G, Ndepth, H, W]
                    pixel_wise_weight_sum = pixel_wise_weight_sum + view_weight.unsqueeze(
                        1
                    )  # [B,1,1,H,W]
                else:
            ...
            if hints is not None and validhints is not None:
                ...
                hints_stage = hints.clone()
                hints_stage = F.interpolate(
                    hints_stage, scale_factor=1 / scale, mode="nearest"
                ).unsqueeze(1)
                validhints_stage = validhints.clone()
                validhints_stage = F.interpolate(
                    validhints_stage, scale_factor=1 / scale, mode="nearest"
                ).unsqueeze(1)
                hints_stage = hints_stage * validhints_stage
                ...
                hints_stage = hints_stage.expand(-1, feats, num_depth, -1, -1)
                validhints_stage = validhints_stage.expand(-1, feats, num_depth, -1, -1)

                # create a tensor of the same size as cost, with disparities
                # between 0 and num_disp-1 along the disparity dimension
                depth_hyps = (
                    depth_sample.unsqueeze(1).expand(batch_size, feats, -1, height, width).detach()
                )
...
class PatchMatch(nn.Module):
    ...
    def __init__(
        ...
        self.evaluation = Evaluation(
            self.G, self.stage, self.evaluate_neighbors, self.patchmatch_iteration
        )
    ...
    def get_propagation_grid(
        ...
        for i in range(len(original_offset)):
            ...
            xy_list.append(
                (xy + torch.cat((offset_x_tensor, offset_y_tensor), dim=1)).unsqueeze(2)
            )
    ...
    def get_evaluation_grid(
        ...
        if self.evaluate_neighbors == 9:  # if 9 neighbors to be sampled in evaluation
            dilation = (
                self.dilation - 1
            )  # dilation of evaluation is a little smaller than propagation
            original_offset = [
                [-dilation, -dilation],
                [-dilation, 0],
                [-dilation, dilation],
    ...
    def forward(
        ...
        if self.propagate_neighbors > 0:
            ...
            if not (self.stage == 1 and self.patchmatch_iteration == 1):
                ...
                propa_offset = propa_offset.view(
                    batch, 2 * self.propagate_neighbors, height * width
                )
                propa_grid = self.get_propagation_grid(
                    batch, height, width, propa_offset, device, img
                )
